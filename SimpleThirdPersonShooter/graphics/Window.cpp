...
int Window::InitWindow(const char* title, int &width, int &height, char** argv, int showCmd)
{
...
bool Window::InitializeWindow(const char* title, HINSTANCE hInstance, int showWnd, int width, int height)
{
	char clsname[25] = "WINDOWCLASS";
	char newtitle[15];
	unsigned int len = strlen(title);
	memcpy(newtitle, title, (len < 10) ? len : 10);
	newtitle[10] = '\0';
	strcat_s(clsname, newtitle);

...
void Sprite::LoadSprite(std::string filename, float width, float height, int numCols, int numRows, double animationSpeed)
{
	if (m_loaded) CleanUp();
	m_loaded = true;
	...
	std::vector<Vertex> vertices = {
		...
		{ glm::vec2(0.0f, height), glm::vec2(0.0f, 1.0f-v) },
		{ glm::vec2(width, height), glm::vec2(u, 1.0f-v) },
		{ glm::vec2(width, 0.0f), glm::vec2(u, 1.0f) },
	};

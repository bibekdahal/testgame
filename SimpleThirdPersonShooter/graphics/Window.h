...
private:
	...
	bool InitializeWindow(const char* title, HINSTANCE hInstance, int showWnd, int width, int height);
...
public:
	Window();
	int InitWindow(const char* title, int &width, int &height, char** argv, int showCmd);
	int MainLoop();
	void SetupOpengl();
	void DestroyOpengl(); 
	...
	bool GetMouseState(int button) { return m_mouseButtons[button]; }
};
